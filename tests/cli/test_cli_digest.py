...
WORKING_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'tlparser', 'workingdir'))
...
def test_digest_command():
    ...
    assert len(new_files) == 1, "Expected exactly one new file, but found: {}".format(new_files)
